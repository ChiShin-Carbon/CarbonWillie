  ...
  const handleCreateBaseline = async () => {
    if (cfvStartDate && cfvEndDate) {
      try {
        ...
        if (response.ok) {
          const data = await response.json()
          const currentBaseline = data.baseline
          const currentReview = currentBaseline.is_completed
          const currentStart = new Date(currentBaseline.cfv_start_date)
          const currentEnd = new Date(currentBaseline.cfv_end_date)
          const newStart = new Date(cfvStartDate)
          const newEnd = new Date(cfvEndDate)
          ...
          if (currentReview === 1 || !currentBaseline) {
            // 若初次盤查或現有基準年盤查已完成 -> 新增基準年設定
            const createResponse = await fetch('http://localhost:8000/baseline', {
              method: 'POST',
            ...
            currentReview === 0 &&
            newStartYear >= currentStartYear &&
            newEndYear <= currentEndYear
          ) {
            // 若現有基準年盤查尚未完成且在現有基準年之年分範圍內 -> 更新基準年設定
            const updateResponse = await fetch(
              `http://localhost:8000/baseline/${currentBaseline.baseline_id}`,
              {
                method: 'PUT',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({
