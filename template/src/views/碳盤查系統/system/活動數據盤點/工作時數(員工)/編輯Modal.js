...
const EditModal = ({ 
            ...
            try {
                ...
                if (response.ok) {
                    ...
                    if (data.employees && data.employees.length > 0) {
                        ...
                        if (employee?.period_date && minMonth && maxMonth) {
                            if (employee.period_date < minMonth || employee.period_date > maxMonth) {
                                showFormAlert(`注意：此記錄的月份 (${formatYearMonthDisplay(employee.period_date)}) 不在當前有效範圍內`, 'warning');
                            }
                        }
                    } else {
    ...
    }, [selectedEmployee, isEditModalVisible, resetStates, minMonth, maxMonth]);
