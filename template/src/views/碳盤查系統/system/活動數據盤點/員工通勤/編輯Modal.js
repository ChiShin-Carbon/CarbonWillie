...
const EditModal = ({ 
    ...
    const safeRefresh = async () => {
        ...
        try {
            ...
            // Then try our local refresh
                ...
                console.log("Using localRefreshCommuteData");
    ...
    // Fetch commute data when selectedCommute changes
    useEffect(() => {
            ...
            try {
                ...
                if (response.ok) {
                    ...
                    if (data.Commute && data.Commute.length > 0) {
                        ...
                        const commute = data.Commute[0];
                        const transportValue = commute?.transportation?.toString() || '1';
                        
                        // Log raw data for debugging
                        console.log("Raw commute data:", commute);
                        console.log("Raw oil_species value:", commute.oil_species);
                        console.log("Type of oil_species:", typeof commute.oil_species);
                        
                        // Convert oil_species to string to ensure consistent handling
                        // If it's a boolean, convert true to "1" and false to "0"
                        let oilSpeciesValue;
                        if (typeof commute.oil_species === 'boolean') {
                            oilSpeciesValue = commute.oil_species ? "1" : "0";
                            console.log("Converting boolean to string:", oilSpeciesValue);
                        } else {
                            // Ensure we have a string value (could be number or string)
                            oilSpeciesValue = commute.oil_species?.toString() || "0";
                        ...
                        setFormValues({
                            ...
                            kilometer: commute?.kilometer?.toString() || '',
                            remark: commute?.remark || '',
                        ...
                        if (commute?.img_path) {
                            ...
                            setExistingImage(commute.img_path);
                            // Set preview with full URL path
                            setPreviewImage(`fastapi/${commute.img_path}`);
                            setUseExistingImage(true);
                        }
                    } else {
                        console.error('No commute data found');
                        showFormAlert('找不到通勤資料', 'danger');
                    }
                } else {
                    console.error('Error fetching commute data:', await response.text());
        ...
        if (isEditModalVisible && selectedCommute) {
            fetchCommuteData();
        }
    }, [selectedCommute, isEditModalVisible, resetStates]);
    ...
    const handleEditSubmit = async (e) => {
        ...
        try {
            ...
            if (response.ok && data.status === "success") {
                ...
                showFormAlert(data.message || "更新通勤記錄失敗。", "danger");
            }
        } catch (error) {
            console.error("更新通勤記錄時發生錯誤:", error);
    ...
    return (
        <CModal
            ...
            onClose={handleClose}
            aria-labelledby="CommuteModalLabel"
            size="xl"
            className={styles.modal}
        >
            <CModalHeader>
                <CModalTitle id="CommuteModalLabel">
                    <b>編輯數據-員工通勤</b>
                </CModalTitle>
            </CModalHeader>
