...
const EditModal = ({
    ...
    const safeRefresh = async () => {
        ...
        try {
            ...
            // Then try our local refresh
                ...
                console.log("Using localrefreshMachineryData");
    ...
    // Fetch machinery data when selectedMachinery changes
    useEffect(() => {
        const fetchMachineryData = async () => {
            ...
            try {
                ...
                if (data.Machinery && data.Machinery.length > 0) {
                    ...
                    setFormValues({
                        date: machineryData?.Doc_date || '',
                        num: machineryData?.Doc_number || '',
                        location: machineryData?.machinery_location || '',
                        type: machineryData?.energy_type || '',
                        usage: machineryData?.usage || '',
                        remark: machineryData?.remark || '',
                        image: null, // Don't set file object here, just track path
                    });

                    if (machineryData?.img_path) {
                        // Store existing image path
                        setExistingImage(machineryData.img_path);
    ...
    const handleEditSubmit = async (e) => {
        ...
        try {
            ...
            if (response.ok && data.status === "success") {
                ...
                showFormAlert(data.message || "更新機具記錄失敗。", "danger");
            }
        } catch (error) {
            console.error("更新機具記錄時發生錯誤:", error);
    ...
    return (
        <CModal
            ...
            onClose={handleClose}
            aria-labelledby="MachineryModalLabel"
            size="xl"
            className={styles.modal}
        >
            <CModalHeader>
                <CModalTitle id="MachineryModalLabel">
                    <b>編輯數據-廠內機具</b>
                </CModalTitle>
            </CModalHeader>
