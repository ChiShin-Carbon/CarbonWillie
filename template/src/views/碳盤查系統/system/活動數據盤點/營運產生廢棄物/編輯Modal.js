...
const EditModal = ({ 
    ...
    const safeRefresh = async () => {
        console.log("Attempting to refresh operational waste data...");
        try {
            // First try the prop passed from parent
            if (typeof refreshWasteData === 'function') {
                console.log("Using refreshWasteData from props");
                await refreshWasteData();
                return true;
            }
            // Then try our local refresh
            else if (localRefreshData && typeof localRefreshData.refreshWasteData === 'function') {
                console.log("Using localrefreshWasteData");
                await localRefreshData.refreshWasteData();
                return true;
            }
    ...
    // Fetch operational waste data when selectedWaste changes
    useEffect(() => {
        const fetchOperationalData = async () => {
            ...
            try {
                ...
                if (response.ok) {
                    ...
                    if (data.Operational_Waste && data.Operational_Waste.length > 0) {
                        // Populate form with fetched data
                        const waste = data.Operational_Waste[0];
                        
                        // Log raw data for debugging
                        console.log("Raw operational waste data:", waste);
                        ...
                        console.error('No operational waste data found');
                        showFormAlert('找不到廢棄物資料', 'danger');
                    }
                } else {
                    console.error('Error fetching operational waste data:', await response.text());
            ...
            if (response.ok && data.status === "success") {
                ...
                setTimeout(async () => {
                    ...
                    if (typeof setCurrentFunction === 'function') {
                        setCurrentFunction("OperationalWaste");
                    }

                    if (typeof setCurrentTitle === 'function') {
                        setCurrentTitle("營運產生廢棄物");
                    }
    ...
    return (
        <CModal
            ...
            onClose={handleClose}
            aria-labelledby="OperationalWasteModalLabel"
            size="xl"
            className={styles.modal}
        >
            <CModalHeader>
                <CModalTitle id="OperationalWasteModalLabel">
                    <b>編輯數據-營運產生廢棄物</b>
                </CModalTitle>
            </CModalHeader>
