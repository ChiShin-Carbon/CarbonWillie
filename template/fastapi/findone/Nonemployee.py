...
NonEmployee_findone = APIRouter()

class NonemployeeRequest(BaseModel):
    Nonemployee_id: int


@NonEmployee_findone.post("/NonEmployee_findone")
def read_employee_data(request: NonemployeeRequest):
    ...
    if conn:
        try:
            ...
            with conn.cursor() as cursor:
                ...
                FROM NonEmployee 
                WHERE NonEmployee.nonemployee_id = ?
                """
                cursor.execute(query, (request.Nonemployee_id,))  # Use the user's input in the query
