...
Selling_waste_findone = APIRouter()
class SWRequest(BaseModel):
    SW_id: int

@Selling_waste_findone.post("/Selling_waste_findone")
def read_user_credentials(request: SWRequest):
    conn = connectDB()  # Establish connection using your custom connect function
    if conn:
        cursor = conn.cursor()
        try:
            # Secure SQL query using a parameterized query to prevent SQL injection
            query = "SELECT * FROM Selling_Waste where waste_id = ?"
            cursor.execute(query, (request.SW_id,))
