...
Emergency_Generator_findone = APIRouter()

class EGRequest(BaseModel):
    EG_id: int


@Emergency_Generator_findone.post("/Emergency_Generator_findone")
def read_user_credentials(request: EGRequest):
    conn = connectDB()  # Establish connection using your custom connect function
    if conn:
        cursor = conn.cursor()
        try:
            # Secure SQL query using a parameterized query to prevent SQL injection
            query = "SELECT * FROM Emergency_Generator WHERE generator_id = ?"
            cursor.execute(query, (request.EG_id,))
