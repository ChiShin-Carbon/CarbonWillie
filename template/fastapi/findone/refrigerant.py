...
refrigerant_findone = APIRouter()

class refrigerantRequest(BaseModel):
    refrigerant_id: int

@refrigerant_findone.get("/refrigerant_findone")
def read_refrigerant(request: refrigerantRequest):
            ...
            for record in refrigerant_records:
                ...
                if record[9]:  # If there is a fill record
                    refrigerants[refrigerant_id]["fillrec"].append({
                        ...
                        "fillrec_edit_time": record[18].strftime('%Y-%m-%d %H:%M'),
                    })
