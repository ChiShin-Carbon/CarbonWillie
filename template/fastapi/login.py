...
def read_user_credentials(user: User):
    ...
    if conn:
        ...
        try:
            ...
            query = "SELECT username, account, password FROM users WHERE account = ? AND password = ?"
            ...
            if user_record:
                ...
                result = {"username": user_record[0], "account": user_record[1], "password": user_record[2]}
                return {"user": result}  
            else:
