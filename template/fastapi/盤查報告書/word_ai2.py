...
def generate_intro(company_info: dict):
    """使用 OpenAI 生成企業前言"""
    ...
    **請注意**：
    ...
    response = openai_client.chat.completions.create(
        model="gpt-4",
        messages=[{"role": "system", "content": "你是專門為企業撰寫正式介紹的專家。"},
...
def generate_summary(company_info: dict):
    """使用 OpenAI 生成企業簡介"""
    openai_client = OpenAI(api_key=os.getenv("OPENAI_API_KEY"))
...
async def generate_company_info(request: CompanyRequest):
    ...
    intro = generate_intro(company_info)
    summary = generate_summary(company_info)
