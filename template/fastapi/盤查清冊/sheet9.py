...
def create_internal_machinery_sheet(wb):
    """建立 '類別一-廠內機具' 工作表"""
    sheet_name = "類別一-廠內機具"
    ...
    # 填入數據
    data = [
        {'machinery_location': '', 'energy_type': '選擇能源類型', 'month': '選擇時期',
         'usage': 0, 'unit': '選擇單位', 'remark': 'none'}
    ] * 3  # 建立 3 行預設數據

    row = 4
    for item in data:
        for col_idx, key in enumerate(['machinery_location', 'energy_type', 'month', 'usage', 'unit', 'remark'], start=1):
