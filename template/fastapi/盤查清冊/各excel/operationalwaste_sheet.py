...
def get_operational_waste_data(year):
    """從API獲取指定年份的營運產生廢棄物資料"""
    try:
        api_url = f"{API_BASE_URL}/operational_waste_data_by_year/{year}"
        response = requests.get(api_url)
        
        if response.status_code == 200:
            return response.json()
        else:
            print(f"獲取營運產生廢棄物資料失敗，狀態碼: {response.status_code}，錯誤訊息: {response.text}")
            return None
    except Exception as e:
        print(f"連接營運產生廢棄物API時發生錯誤: {e}")
        return None

def create_operational_waste_sheet(wb, data=None):
    """建立 '類別三-營運產生廢棄物' 工作表
    """
    if data is None:
        data=[]
    
    sheet_name = "類別三-營運產生廢棄物"
    ws = wb.create_sheet(title=sheet_name)

    # 合併標題列
    ws.merge_cells(start_row=1, start_column=1, end_row=1, end_column=2)
    merged_cell = ws.cell(row=1, column=1, value="營運產生廢棄物")
